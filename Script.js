(() => {
  ...
  const CONFIG = {
    ...
    inviteUrl: '',
    // Caminho do áudio real de ondas do mar (opcional). Se o arquivo existir
    // em audio/waves.mp3, ele é usado. Se não existir, o site cai automaticamente
    // para o som gerado por código, sem quebrar nada.
    waveAudioFile: 'audio/waves.mp3',
  };
  ...
  function initSound(){
    ...
    let realAudio = null;         // <audio> real, se o arquivo existir
    let usingRealAudio = null;    // null = ainda não decidido, true/false depois
    let sourceCheck = null;       // promessa única de verificação

    /* -------- fonte 1: arquivo de áudio real (audio/waves.mp3) -------- */
    function checkRealAudioFile(){
      if (sourceCheck) return sourceCheck;
      sourceCheck = new Promise((resolve) => {
        if (!CONFIG.waveAudioFile){ resolve(false); return; }
        const el = new Audio();
        let settled = false;
        const finish = (ok) => {
          if (settled) return;
        ...
        el.src = CONFIG.waveAudioFile;
        el.load();
        // se em 2.5s não confirmou nada (ex: servidor lento), assume que não tem arquivo
        setTimeout(() => finish(false), 2500);
      });
      return sourceCheck;
    }

    /* -------- fonte 2 (reserva): som gerado por código, sem arquivo -------- */
    ...
    function fadeElement(el, target, duration){
    ...
    async function setPlaying(playing){
      ...
      if (usingRealAudio === null){
        usingRealAudio = await checkRealAudioFile();
      }

      if (usingRealAudio){
        if (playing){
          realAudio.play().catch(() => {});
          fadeElement(realAudio, 0.55, 1200);
        } else {
          fadeElement(realAudio, 0, 800);
        }
      } else {
        ensureSynthContext();
        const target = playing ? 0.09 : 0;
